...
module.exports = function () {
    ...
    return {
        ...
        createOrders: function (orderOrOrderArray, callback) {
            ...
            _.forEach(orderArrayToSend, function (order) {
                ...
                if (orderValidation !== 'SUCCESS') {
                    validationErrors.push({OrderNumber: order.OrderNumber, Error: orderValidation});
                    return false;
                }
            });
            if (validationErrors.length) {
                return callback(new Error('PRE_VALIDATION_ERROR'), {"ProLogCode" : 'PRE_VALIDATION_ERROR', "ProLogMessage": "Order(s) Formatted incorrectly see order array.", "OrderResults": validationErrors});
            }
        ...
        deleteOrder : function (OrderNumber, callback) {

            var argumentsToSend = {OrderNumber: OrderNumber};

            proLog('PLDeleteOrder', argumentsToSend, function (err, result) {
                if (err) return callback(err, result);
                debugLog( 'Inspecting -> DeleteResults',  result );
                return callback(null, result);
            });
        ...
        createASN: function (ASN, callback) {
            ...
            if (asnValidation !== 'SUCCESS') {
                return callback(new Error('PRE_VALIDATION_ERROR'), {"ProLogCode" : 'PRE_VALIDATION_ERROR', "ProLogMessage": JSON.stringify(asnValidation)});
            }

            // 'OrderLines' remapping to match SOAP requirement
            var remappedLines = [];
            _.forEach(ASN.Lines, function (line) {
                remappedLines.push({'PLASNLine': line});
            });
            ASN.Lines = remappedLines;
            ...
            proLog('PLSubmitASN', argumentsToSend, function (err, result) {
                if (err) return callback(err, result);
                debugLog( 'Inspecting -> ASNresults',  result );
                return callback(null, result);
            });
        },
        ...
        getASNStatus: function (type, value, callback) {
            ...
            proLog('PLGetASNStatus', argumentsToSend, function (err, result) {
                ...
                var Lines = result.ASN.Lines.PLASNStatusLine;
                var ASNLines = [];

                var ASNresults = _.pick(result.ASN, propertiesToPick_ASN);
    ...
    function validateOrder(order) {
        ...
        var orderSchema = {
            ...
            "properties": {
                ...
                "PartialShip": {"type": "boolean"},
                "SplitShip": {"type": "boolean"},
                "ShippingService": {"type": "string"},
                "BillThirdParty": {"type": "boolean"},
                "SaturdayDelivery" : {"type": "boolean"},
                "Residential": {"type": "boolean"},
                "InsurePackages": {"type": "boolean"},
                "InsureThreshold": {"type": "number"},
                "PackingListTemplate": {"type": "string"},
                "PackingListComment": {"type": "string"},
